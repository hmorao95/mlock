...
%
% Syntax:
%  root = matlabdeps.internal.commonAncestor(absPaths) Return the common
...
if isempty(absPaths)
    error('matlabdeps:commonAncestor:empty', 'No paths given.');
end
...
for i = 1:numel(absPaths)
    folders(i) = matlabdeps.internal.toPosix(fileparts(char(absPaths(i))));
end
...
if strlength(root) == 0
    error('matlabdeps:commonAncestor:none', ...
